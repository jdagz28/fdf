...
/*   Updated: 2023/06/19 12:11:46 by jdagoy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
										...
										t_fdf *fdf)
{
	...
	while (*j < fdf->map->size_y)
	{
		...
		{
			rotated_pt = apply_rotation_matrix_to_point(pt, fdf->display);
			if (rotated_pt->z > fdf->display->screen_depth)
				res[i][*j] = project_perspective(pt, \
								fdf->display->screen_plane, fdf->display);
			else
				res[i][*j] = NULL;
			free (rotated_pt);
		}
